...
#' @param ret_input If TRUE, return plot input.
...
#' @param var_cl Variable name for specifying heatmap annotation.
...
ms_plot_hm <- function(
      ...
      cl2 <- as.character(
        unlist(
          lapply(
            ...
            function(i) {
              unique(rowData(exp1)[
                rowData(exp1)[[feat_var]] == colnames(h_in)[[i]],
              ][[var_cl]])
            }
          )
        )
      ...
      cl2 <- as.factor(
        unlist(
          lapply(
            seq.int(1, length(colnames(h_in)), 1),
            function(i) {
              unique(h[h[["Name"]] == colnames(h_in)[[i]], ][["Class"]])
            }
          )
        )
      )
  ...
  h_out <- ComplexHeatmap::Heatmap(
    h_in,
    top_annotation = if (!is.null(var_cl)) hm_anno_col else NULL,
    col = fun_hm_col,
    name = legend1,
