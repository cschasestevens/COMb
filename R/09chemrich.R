...
#' generated by ms_stat_uni(). The original enrichment method is adapted
...
#'
#' @param d_stat Univariate results list generated by ms_stat_uni.
...
ms_stat_crich <- function(
  ...
  cl2 <- col_lab
  cp1 <- col_pvalue
  # Add name column to reference data
  d2[["Name"]] <- d2[[cl2]]
  ...
  #---- Functions ----
  fun.stat.chemrich <- function(x) { # nolint
    ...
    cr2 <- setNames(
      lapply(
        ...
        function(y) {
          print(paste("Running KS test for:", cr1[[y]]))
          # KS test
          ksdf <- ks.test(
            d3[
  ...
  fun.run.chemrich <- function() { # nolint
    if (Sys.info()[["sysname"]] != "Windows") {
      cr_run <- dplyr::bind_rows(setNames(
        parallel::mclapply(
          mc.cores = ceiling(parallel::detectCores() / 4),
          seq.int(1, length(cmp1), 1),
          function(x) {
            cr3 <- fun.stat.chemrich(x)
            return(cr3) # nolint
          }
        ),
        cmp1
